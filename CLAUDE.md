...
- **Throwaway Claude sessions:** `claude()` in `functions.zsh` always runs from `~/claudetmp`, passing the launch folder with `--add-dir` (skipped for `$HOME`). `CLAUDE_NO_TMP=1 claude` bypasses it. Refresh the snippet cache after editing: `zinit update "$ZSH_HOME/functions.zsh"`

...
2. `zinit.zsh` - loads modules via zinit snippets/plugins in this order:
   ...
   - `zsh-completions`, then `compinit` (with `-C`, a full compaudit run happens once a day keyed on the dump's mtime), then fzf-tab, copy-pasta, hhighlighter deferred with `wait` (zinit's turbo mode; there is no `turbo` ice)

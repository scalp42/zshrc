...
# Generated by Powerlevel10k configuration wizard on 2020-10-02 at 01:43 PDT.
...
() {
  ...
  typeset -g POWERLEVEL9K_VCS_UNTRACKED_ICON='untracked '
  ...
  function my_git_formatter() {
    ...
    if (( $1 )); then
      ...
      local added='%%244F'      # grey foreground
    fi
    ...
    res+="${clean}${where//\%/%%}"
    [[ -n ${VCS_STATUS_COMMIT} ]] && res+=" %255F[%226F${VCS_STATUS_COMMIT[0,7]}%255F]"
    [[ ! -n ${VCS_STATUS_COMMIT} ]] && res+=" %255F[%226Finit%255F]"

    # Display the current Git commit if there is no branch or tag.
    # Tip: To always display the current Git commit, remove `[[ -z $where ]] &&` from the next line.
    # [[ -z $where ]] && res+="${meta}@${clean}${VCS_STATUS_COMMIT[1,8]}"
    [[ -z $where ]] && res+=" detached"
    ...
    (( VCS_STATUS_NUM_STAGED     )) && res+=" ${added}added ${VCS_STATUS_NUM_STAGED}"
    # !42 if have unstaged changes.
    (( VCS_STATUS_NUM_UNSTAGED   )) && res+=" ${modified}modified ${VCS_STATUS_NUM_UNSTAGED}"
  ...
  ####################################[ time: current time ]####################################
  # Current time color.
  typeset -g POWERLEVEL9K_TIME_FOREGROUND=229
  # Format for the current time: 09:51:02. See `man 3 strftime`.
  typeset -g POWERLEVEL9K_TIME_FORMAT='%B%D{%H:%M:%S}'
